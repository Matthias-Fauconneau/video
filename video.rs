...
fn profile_tier_level(s: &mut BitReader, max_layers: usize) {
    ...
    if max_layers > 1 {
        ...
        let _layers = layers.map(|(profile, level)| {
            if profile { s.bits(44); s.bits(44); }
            if level { s.u8(); }
        });
    }
}
...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    let path = std::env::args().skip(1).next().unwrap_or(std::env::var("HOME")?+"/sample.mkv");
                    ...
                    match unit_type {
                        ...
                        VPS => {
                            ...
                            if s.bit() {
                                ...
                                (0..s.ue()).map(|i| { let _hrd_layer_set_index = s.ue(); hrd_parameters(s, i > 0 && s.bit(), max_layers) })
                            }
                            s.bit(); // vps_extension*/
                        }
                        SPS => {
                            println!("SPS");
                            let _vps = s.bits(4) as usize;
                            let max_layers = (s.bits(3) + 1) as usize;
                            let _temporal_id_nesting = s.bit();
                            profile_tier_level(s, max_layers);
                            let id = s.ue() as usize;
                            assert!(id <= 16, "{id}");
                            assert_eq!(id, sps.len());
                            ...
                            sps[id] = SPS{separate_colour_plane, width, height, log2_max_poc_lsb};
                        },
                        PPS => {println!("PPS"); assert!(pps.is_empty()); pps.push(PPS{
                            pps: s.ue() as usize,
                            sps: s.ue() as usize,
                        ...
                        IDR_N_LP|TRAIL_R|TRAIL_N => {
                            ...
                            fn IRAP(unit_type: u8) -> bool { 16 <= unit_type && unit_type <= 23 }
                            if IRAP(unit_type) { let _no_output_of_prior_pics = s.bit(); }
                            let ref pps = pps[s.ue() as usize];
                            let ref sps = sps[pps.sps];
